/* Данный файл отвечает за создание основного окна приложения. В конструкторе содержится метод setupServer(), который создает              *
 * экземпляр ServerManager, прослушивающий подключения клиентов и обрабатывающий события, связанные с подключением, отключением,           *
 * и получением текстовых сообщений, статусов и файлов, который позволяет управлять состоянием клиента и его взаимодействия с сервером.    *
 * Посредством действий на сервере или действий осуществляемых клиентом, осуществляется отработка слотов, определенных в данном файле,     *
 * связанных с изменением поведения графических элементов.                                                                                 */
...
void MainWindow::newClientConnected(QTcpSocket *client)
{
    ...
    ui->lstClients->addItem(QString("Клиент добавлен: %1").arg(id));
    auto chatWidget= new ClientChatWidget(client, ui->tbClientsChat);
    ui->tbClientsChat->addTab(chatWidget, QString("Клиент (%1)").arg(id));
