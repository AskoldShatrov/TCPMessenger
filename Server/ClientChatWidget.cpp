/* Файл отвечает за создание виджета для взаимодействия с отдельным клиентом. В конструкторе создается экземпляр ClientManager, *
 *  который управляет соединением с клиентом и обрабатывает различные события, связанные с клиентом.                            */
...
{
    emit isTyping(QString("%1 печатает...").arg(_client->name()));
}

void ClientChatWidget::onInitReceivingFile(QString clientName, QString fileName, qint64 fileSize)
{
    auto message = QString("Клиет (%1) хочет отправить файл. Принять?\nFile Name:%2\nFile Size: %3 bytes")
