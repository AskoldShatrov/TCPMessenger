/* Класс отвечает за управление сервером, включая прием новых клиентов, обработку их сообщений и уведомление других клиентов о событиях в системе. *
*  Метод setupServer инициализирует сервер, который начинает прослушивать порт для новых подключений и получения указателя на QTcpSocket клиента.  *
*  Клиенты добавляются в список клентов с использованием контейнера QMap. Также посредством методов данного класса осущесвтяляется уведомление     *
*  других клиентов о подключении нового клиента и пересылка сообщений от одного клиента к другому, если те выбирает общение между друг другом      */
