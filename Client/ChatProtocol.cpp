/** Класс предоставляет набор методов для создания сообщений разных типов: текстовых, сигналов о печати, об изменения имени и статуса клиента,      **
 ** инициализации передачи файла, подтверждения и отказа от приема файла. Каждый из этих методов формирует соответствующий байтовый массива,        **
 ** который затем может быть отправлен через сокет. Метод loadData(QByteArray data) используется для чтения и интерпретации полученных данных.      **
 ** Он определяет тип сообщения на основе заголовка и загружает данные в поля класса, что позволяет извлекать информацию о типе сообщения. Также    **
 ** класс содержит набор геттеров для соблюдения принципа инкапсуляции и использования данных из ClientManager                                      **/
