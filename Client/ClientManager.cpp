/** Класс отвечает за управление сетевым соединением, включая подключение и отключение от сервера посредством создания экземпляра _socket   **
 ** класса QTcpSocket, который использует стандартные сигналы и методы своего класса для подключения к локальному хосту, излучения сигналов **
 ** об успешном подключении или отключении клиента, а также для записи или считывания разных типов данных, заданных в перечислениях         **
 ** класса ChatProtocol.cpp с использованием методов данного класса. Слот readyRead() вызывается при поступлении данных от сервера.         **
 ** Он декодирует данные с помощью объекта ChatProtocol, определяющим тип полученного сообщения и генерирует соответствующие сигналы,       **
 ** которые могут быть обработаны в MainWindow.                                                                                             **/
